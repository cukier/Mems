...
// 5x7 bitmap font: full uppercase A-Z, digits, and the punctuation a
// question statement needs ('?' '!' '=' ',' '+' '-' '.' ':' space). No
// lowercase glyphs — font_lookup() upper-cases first, same shape either way,
// which keeps this table a fraction of a full-ASCII font's size. No accented
// Latin letters either (á/ã/é/ç/...); an accented char falls through to
// space, so send unaccented statement text from the console. Each glyph is 7
// rows, top to bottom; each byte's bits 4..0 are columns 0..4 (bit4 =
// leftmost column), so a row like "01110" reads directly as 0x0E.
typedef struct {
  char ch;
  uint8_t rows[7];
} font_glyph_t;
...
static const uint8_t *font_lookup(char c) {
  if (c >= 'a' && c <= 'z') {
    c = (char)(c - 'a' + 'A'); // no lowercase glyphs; same shape upper-cased
  }
  ...
  return s_font[FONT_GLYPH_COUNT - 1].rows; // unknown char (incl. accents) -> space
}
