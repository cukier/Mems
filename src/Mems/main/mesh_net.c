...
_Static_assert(sizeof(mesh_pkt_t) == 24, "mesh packet grew past the ADV budget");
...
static bool seen_mark_and_check(uint16_t node_id, uint8_t seq) {
    ...
    s_seen[slot] = (typeof(s_seen[0])){.node_id = node_id, .last_seq = seq, .valid = true};
    return false;
}
...
static void broadcast_one(const mesh_pkt_t *pkt) {
    ...
    ble_gap_adv_start(own_addr_type, NULL, ADV_CYCLE_MS, &adv_params, gap_event_cb, NULL);
}
...
static void handle_discovered_packet(const mesh_pkt_t *pkt) {
    ...
    ESP_LOGI(TAG, "node %04x seq=%u ttl=%u accel_mg=[%d %d %d] gyro_dps10=[%d %d %d] rpy_dd=[%d %d %d]",
             pkt->node_id, pkt->seq, pkt->ttl,
             pkt->accel_mg[0], pkt->accel_mg[1], pkt->accel_mg[2],
             pkt->gyro_dps10[0], pkt->gyro_dps10[1], pkt->gyro_dps10[2],
             pkt->rpy_dd[0], pkt->rpy_dd[1], pkt->rpy_dd[2]);
...
static void start_scan(void) {
    ...
    disc_params.filter_duplicates = 0; // we need every packet, not just the first per address
