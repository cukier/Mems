...
#define REG_OUTX_L_G   0x22 // gyro output, 6 bytes (X,Y,Z), followed by accel at 0x28
...
static esp_err_t reg_write(uint8_t reg, uint8_t val) {
    ...
    return i2c_master_write_to_device(s_port, LSM6DS3_I2C_ADDR, buf, sizeof(buf), pdMS_TO_TICKS(100));
}

static esp_err_t reg_read(uint8_t reg, uint8_t *data, size_t len) {
    return i2c_master_write_read_device(s_port, LSM6DS3_I2C_ADDR, &reg, 1, data, len, pdMS_TO_TICKS(100));
}

esp_err_t lsm6ds3_init(i2c_port_t port, gpio_num_t sda_gpio, gpio_num_t scl_gpio, uint32_t clk_speed_hz) {
    s_port = port;
    ...
    // BDU=1 (block data update, avoids reading torn samples), IF_INC=1 (auto-increment address).
